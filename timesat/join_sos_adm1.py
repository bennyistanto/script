...
dbf_folder = "Z:\\Temp\\TIMESAT\\DBF\\adm1\\SOS\\2019_2020"
csv_folder = "Z:\\Temp\\TIMESAT\\DBF\\adm1\\SOS"
...
file_concat = pd.concat([pd.read_csv(f).set_index('A1TEXT') for f in glob.glob(path)],axis=1).reset_index()
...
csv_sorted.to_csv(os.path.join(csv_folder, "sos.csv"), index=False)
