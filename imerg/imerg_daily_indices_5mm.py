...
def create_WD_List(_WD_folder):
    ...
    print("looking for file with naming wld_cli_wet_5mm_imerg_YYYYMMDD")
...
def execute_first_WD(_tiffolder, _WDFolder, threshold):
    ...
    WDFilename = 'wld_cli_wet_5mm_imerg_{0}{1}{2}.tif'.format(WDyear.zfill(4), WDmonth.zfill(2), WDday.zfill(2))
...
def execute_WD(_lastdate, _tiffolder, _WD_folder, threshold):
    ...
    date_formatted = date(int(_lastdate[0:4]), int(_lastdate[4:6]), int(_lastdate[6:8]))
    last_wetname = 'wld_cli_wet_5mm_imerg_{0}'.format(_lastdate)
    ...
    if arcpy.Exists(next_dailydata):
        ...
        new_wet_name = 'wld_cli_wet_5mm_imerg_{0}{1}{2}.tif'.format(WDyear1.zfill(4), WDmonth1.zfill(2), WDday1.zfill(2))
...
if __name__ == '__main__':
    ...
    with arcpy.EnvManager(scratchWorkspace=r"X:\ArcGIS_TEMP\Scratch.gdb", \
        ...
        create_CWD('X:\\Temp\\imerg\\products\\cwd\\cwd_5mm_temp',\
            'X:\\Temp\\imerg\\data\\geotiff\\rainfall_1days', 5)
        # Run the function (output folder, input folder, threshold)
        create_WD('X:\\Temp\\imerg\\products\\wetdays\\wetdays_5mm_temp',\
            'X:\\Temp\\imerg\\data\\geotiff\\rainfall_1days', 5)